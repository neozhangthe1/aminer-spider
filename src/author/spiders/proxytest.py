...
class Titlespider(BaseSpider):
    ...
    def start_requests(self):
      ...
      for url in self.start_urls:
        req = Request(url,dont_filter = True,callback = self.parse,meta={'proxy':self.proxy,'dont_retry':True,'dont_redirect':True,'User-agent':random.choice(AGENTS),'aid':self.start_aid,'handle_httpstatus_list':[302,503,200,307,403,404,502,500,504,400],'pointer':self.aidpointer})
    ...
    def parse(self, response):
      ...
      if response.status != 200 and response.status!=404 :
        ...
        if response.meta['proxy'] == self.proxy:
          self.changep()      
        request = Request(response.url, dont_filter = True,callback = self.parse, meta = {'dont_redirect':True,'dont_retry':True,'User-agent':random.choice(AGENTS),'aid':self.start_aid,'proxy':self.proxy,'handle_httpstatus_list':[302,503,200,307,403,404,502,500,504,400],'cookie_jar': cookieJar,'pointer':pointer})
      ...
      if response.status ==200: 
        ...
        print '!!!!!!!!!!!!!!!!!!!!!!!'
        print 'extract a new proxy!!!!'
        if response.meta['proxy'] not in Proxypool:
    ...
    def start_next(self,iid):
      ...
      if _aid:
        ...
        request1 = Request("http://scholar.google.com/scholar?start=0&q=%s&hl=en&num=100&as_sdt=0,5"%query,dont_filter = True, callback = self.parse,meta={'User-agent':random.choice(AGENTS),'aid':_aid,'proxy':self.proxy,'dont_retry':True,'dont_redirect':True,'handle_httpstatus_list':[302,503,200,307,403,404,502,500,504,400],'pointer':pointer})
        request2 = Request("http://scholar.google.com/scholar?start=500&q=%s&hl=en&num=100&as_sdt=0,5"%query,dont_filter = True, callback = self.parse,meta={'User-agent':random.choice(AGENTS),'aid':_aid,'proxy':self.proxy,'dont_retry':True,'dont_redirect':True,'handle_httpstatus_list':[302,503,200,307,403,404,502,500,504,400],'pointer':pointer})
